 ...
 @extends('layouts.app')
 @section('content')
<div class="bg-gray-800 text-white font-bold p-4 flex items-center justify-between ">
          ...
          <tr class="hover:bg-blue-100">
            ...
            <td class="p-2 flex justify-around">
              <a href="{{  url('/layouts/'.$Empleado->id.'/edit') }}">
                <button class="bg-gray-800 text-white rounded px-2 py-1 hover:bg-blue-500">Editar</button>
              </a>
              <form action="{{ url('/layouts/'.$Empleado->id)  }}" method="post">
                @csrf  
                {{ method_field('DELETE') }}
                <button class="bg-red-500 text-white rounded px-2 py-1 hover:bg-red-700" onclick="return confirm('estas seguro que quieres eliminar este Empleado?')">Eliminar </button>
              </form>
            </td>
          </tr>
          @endforeach
